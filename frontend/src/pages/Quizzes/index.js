...
export default function Quizzes() {
    ...
    useEffect(() => {
        ...
        setFinalData(data)
        // console.log('answers: ', answers, yourScore, allCorrect)
    },
    [answers])

    const updateSessionUnmount = async (beginTime) => {
        await axios
        .put(`${HOST}api/session/${session?._id}`, { timeTracker: {...timeTracker, quizzes: {beginTime: beginTime, endTime: new Date(), type: "closeWindow"}}, preQuiz: [...result, { answers, yourScore, allCorrect, time: new Date()}], timeLastUpdated: new Date()}, {new: true})
        .then(data => {
            // sessionStorage.setItem('disesSession', JSON.stringify(data.data));
                            ...
                            <Form.Group>
                                ...
                                <div className={styles.radio} key={question.question}>
                                    <div key={question.id} className="inputFrame" style={{ backgroundColor: !submitted ? "white" : submitted && question.answer == ALPHABET[previousAnswers[idx + 1] - 1] ? "#b7e597" : "#ffe7aa"}}>
                                        <div style={{fontSize: "1.3rem", fontWeight: "600", marginBottom: "8px"}}>Question <span style={{ color: "#0065ff", fontSize: "1.4rem"}}># {idx + 1}</span></div>
