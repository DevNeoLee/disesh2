...
const Instruction = () => {
  ...
  useEffect(() => {
    setTimeTracker(prev => ({...prev, instruction: {...prev.instruction, beginTime: beginTime}}))
  }, [setTimeTracker])

  const updateSessionUnmount = async (clickTabArray, beginTime) => {
    await axios
    .put(`${HOST}api/session/${session?._id}`, { timeTracker:{...timeTracker, instruction: {beginTime: beginTime, endTime: new Date(), tabChoice: clickTabArray, type: "closeWindow"}}}, {new: true})
    .then(data => {
  ...
  const updateSessionInstruction = async (endTime, id) => {
    await axios
    ...
    .then(data => {
        sessionStorage.setItem('disesSession', JSON.stringify(data.data));
        console.log('updatedSession from Instruction updateSessionInstruction:: ', data.data)
        return data.data
    })
    .catch(err => console.log(err))
  }
  ...
  const handleClick = () => {
    ...
    setTimeTracker(prev => ({...prev, instruction: {...prev.instruction, endTime: endTime, tabChoice: clickTabArray}}))
    updateSessionInstruction(endTime, session?._id)
  }
        ...
        <div className={styles.content}>
                        ...
                        <Tab eventKey="video" title="Watch" width="50%">
                            <div className={styles.videoContainer}>
                                <video ref={videoRef} width="100%" controls>
                                    {
                                        <source src="https://dises2-93fb98d8241f.herokuapp.com/0_disesVideoBaseline_Final.mp4" type="video/mp4" />
                                    }
                                </video>
                            </div>
                        </Tab>
            ...
            <div className={styles.buttonContainer}>
                <ul className={styles.paragraph}>
                    ...
                    <li>  All questions need to be correctly answered to proceed to the decision exercises. </li>
