...
export default function WaitingRoom2({ notifyParticipantLeft, gameState, clientCount, setClientCount, waitingRoomTime, session, setSession}) {
    ...
    useEffect(() => {
        if (waitingroomTime > 0) {
          // Set an interval that decreases secondsLeft every second
          const timer = setInterval(() => {
            setWaitingroomTime(prev => prev - 1);
          }, 1000);
          return () => clearInterval(timer); // Clear the interval on component unmount or when the time runs out
        } 
    }, [waitingroomTime]);
        ...
        <div className={styles.breadcrumbContainer}><Bar now={now}/></div>
            <div className={styles.content}>
                <div className={styles.box}>
                    ...
                    <div style={{ display: "flex", fontSize: "22px"}}><p style={{ fontSize: "22px"}}>Thank you </p><p style={{ color: "#0065ff", fontSize: "22px"}}> for your patience!</p></div>
                    <p style={{ fontSize: "22px"}}>Please wait until other participants have read the instruction and completed the quiz. Note that this could take a few minutes.</p>
                    <div className={styles.waitingMessage} style={{ fontSize: "22px"}}>We are waiting for <span style={{ fontSize: "26px", color: "#0065ff"}}>{5 - clientCount} </span>more participants to form a group…..</div>
                    <p style={{ fontSize: "22px"}}>As soon as a group is formed, Exercise 2 will start automatically.</p>
