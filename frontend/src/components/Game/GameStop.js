...
const GameStop = () => {
    const { role, gameState, setGameState, choice, setChoice, socket, setTotalGroupWater, scores, setChoiceList, choiceList, extraScores, showFinalResultTable, finalScores, showGameStop, gameStopDuration} = useContext(GameContext);
    useEffect(() => {
        console.log('gameState.isDepletedFirstPart: gameState.roundIndex', gameState.isDepletedFirstPart, gameState.roundIndex)
        console.log('gameState.isDepletedSecondPart: gameState.roundIndex', gameState.isDepletedSecondPart, gameState.roundIndex)
    ...
    const CompletedSecondPartMessage = () => {
        return (
            <div className={styles.box}>
                <h2>You have completed the two decision exercises successfully</h2>
                <div className={styles.grayMessage}>
